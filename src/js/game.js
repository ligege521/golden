...
import { Golden, Boom, Ghost } from './props';
...
export default class Game {
    ...
    initSkyBackground () {
        ...
        const { background1, coinsContainer } = resources;
    ...
    initLandBackground () {
        ...
        const { buttonBack, buttonShare, textBottomChanceOut } = resources;
        ...
        this._backgroundContainer_2.on('pointertap', () => {
            if (this.goldenHunter.ropeStop) {
                this.goldenHunter.hookStop = true;
                this.goldenHunter.ropeStop = false;
                this.goldenHunter.roteExtend = true;
     ...
     */
    // initStorage () {
    //     this.storageBox = new PIXI.Graphics();
    //     this.storageBox.beginFill(0x64371f);
    //     this.storageBox.drawRect(this._app.stage.width / 2 - 200 * this.scale, 380 * this.scale, 400 * this.scale, 200 * this.scale);
    //     this.storageBox.endFill();
    //     this._app.stage.addChild(this.storageBox);
    // }
            ...
            if (!this.goldenHunter.ropeStop) {
                if (this.goldenHunter.roteExtend) {
                    ...
                    if (rope.height >= this.goldenHunter.ropeInitHeight * this.scale) {
                        rope.height -= 1;
                        goldenHook.sprite.y -= 1;
                        // let prop = this.goldenArea.list[this.goldenArea.hitIndex];
                        ...
                        // 钩子回来之后清空掉勾掉的元素
                        // this.storageBox.addChild(this._ropeContainer.getChildByName('hitSprite'));
    ...
    updataProps () {
        ...
        for (let i = 0; i < sum; i++) {
            ...
            switch (this.cells[i].type) {
                case 'coin':
                    prop = new Golden(i, this.cells[i].amount, x, y, this.scale, this.propsContainer);
                    prop.type = 'golden';
                    break;
                case 'landmine':
                    prop = new Boom(i, x, y, this.scale, this.propsContainer);
                    prop.type = 'boom';
                    break;
                case 'lootbox':
                    prop = new Boom(i, x, y, this.scale, this.propsContainer);
                    ...
                    break;
                case 'blank':
                    prop = new Ghost(i, x, y, this.scale, this.propsContainer);
                    break;
            }
    ...
    */
    initBlood () {
        // 血条容器
        this.bloodBox = new PIXI.Container();
        this.bloodBox.position.set(this._app.stage.width - 150, 30);
        // 灰色血条
        ...
        this.health.beginFill(0xfb544c);
        this.health.drawRoundedRect(1, 1, this.blood, 18, 10);
        // 添加容器
        this.bloodBox.addChild(bloodBg);
        this.bloodBox.addChild(this.health);
        this._app.stage.addChild(this.bloodBox);
    };
    /*
    *血量减少
    * @parma {number}  要减去的血量
    */
    clearBlood (number) {
        ...
        let bloodWidth = this.blood - number;
        this.blood = bloodWidth;
        health.clear();
    ...
    showPropDialog (prop) {
        ...
        const { gold1, gold2, gold3, gold4, dialogBoom, dialogBagGoldenFull, dialogBagHongBaoFull } = resources;
        ...
        // 禁用游戏点击的点击
        this._backgroundContainer_2.interactive = false;
    ...
    distinguish (prop, cell, ropeHeight) {
        ...
        let empty = this.cells.some(item => (item.type === 'coin' && item.visible));
        console.log(empty);
        if (!empty) {
            this.showNextDialog();
            return;
        ...
        // 初始化钩子勾到的元素
        // this._ropeContainer.removeChild(this._ropeContainer.getChildByName('hitSprite'));
        const { gold1, gold2, gold3, gold4, boom, goldenHook } = resources;
    ...
    initOverDialog () {
        ...
        this._app.stage.addChild(this.gameOverContainer);
        this.gameOverContainer.visible = false;
        this.gameOverContainer.width = this._app.stage.width;
        this.gameOverContainer.height = this._app.stage.height;
        // 黑色框
        let graphics = new PIXI.Graphics();
        graphics.beginFill(0x000033);
        graphics.drawRect(0, 0, this._app.stage.width, this._app.stage.height);
        graphics.endFill();
        graphics.alpha = 0.5;
        this.gameOverContainer.addChild(graphics);
        // 字体
        ...
        message.x = this._app.stage.width / 2 - message.width / 2;
        message.y = this._app.stage.height / 2 - message.height / 2;
        this.gameOverContainer.addChild(message);
        this.gameOverContainer.interactive = true;
        this.gameOverContainer.on('pointertap', () => {
            this.resetGame();
            this.gameOverContainer.visible = false;
            this._backgroundContainer_2.interactive = true;
        });
    ...
    initNextDialog () {
        ...
        this._app.stage.addChild(this.nextContainer);
        this.nextContainer.visible = false;
        this.nextContainer.width = this._app.stage.width;
        this.nextContainer.height = this._app.stage.height;
        // 黑色框
        let graphics = new PIXI.Graphics();
        graphics.beginFill(0x000033);
        graphics.drawRect(0, 0, this._app.stage.width, this._app.stage.height);
        graphics.endFill();
        graphics.alpha = 0.5;
        this.nextContainer.addChild(graphics);
        // 字体
        let style = new PIXI.TextStyle({
            fontFamily: 'PingFangSC-Regular, sans-serif',
            fontWeight: 'bold',
        ...
        let message = new PIXI.Text('下一关', style);
        message.x = this._app.stage.width / 2 - message.width / 2;
        message.y = this._app.stage.height / 2 - message.height / 2;
        this.nextContainer.addChild(message);
        this.nextContainer.interactive = true;
        this.nextContainer.on('pointertap', () => {
            this.nextLevel();
            this.nextContainer.visible = false;
            this._backgroundContainer_2.interactive = true;
        });
    ...
    nextLevel () {
        this.cells = [
            ...
            { type: 'coin', amount: 3, visible: true },
            { type: 'coin', amount: 4, visible: true },
            { type: 'landmine', amount: 4, visible: true },
            { type: 'coin', amount: 4, visible: true },
            { type: 'landmine', amount: 2 },
            { type: 'coin', amount: 2, visible: true },
            { type: 'landmine' },
